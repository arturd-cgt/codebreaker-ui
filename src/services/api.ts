...
class CodebreakerAPI {
  ...
  async joinGame(gameId: string): Promise<JoinGameResponse> {
    const response = await this.client.post<JoinGameResponse>(
      `/games/${gameId}/join`
    );
    return response.data;
  }
  ...
  async submitGuess(
    ...
    const response = await this.client.post<GuessResponse>(
      `/games/${gameId}/guess`,
      data
    );
    return response.data;
  ...
  async getGameState(gameId: string): Promise<GameStateResponse> {
    const response = await this.client.get<GameStateResponse>(
      `/games/${gameId}`
    );
    return response.data;
  }
